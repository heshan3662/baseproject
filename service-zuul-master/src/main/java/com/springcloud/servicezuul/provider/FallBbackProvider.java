package com.springcloud.servicezuul.provider;
...
public class FallBbackProvider implements FallbackProvider {
    ...
    /**
     * return - 返回fallback处理哪一个服务。返回的是服务的名称 ,'*'代表为全部的服务
    ...
    /**
     * 调用子接口新定义的fallback处理逻辑。
     */
    @Override
    public ClientHttpResponse fallbackResponse(String route, Throwable cause) {
        System.out.println("ClientHttpResponse fallbackResponse()");

         DataRes data = new DataRes (route, ResultCode.RESULE_TIMEOUT,"服务正忙，请稍后重试");


        return this.executeFallback(HttpStatus.OK, data.toString(),
